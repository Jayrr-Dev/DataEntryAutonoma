...
<#
.SYNOPSIS
    Compiles manageInput.ahk into a standalone ManageInput.exe (no AHK install required on target PCs).

.DESCRIPTION
    Uses Ahk2Exe with the AutoHotkey v2 64-bit base interpreter. Output goes to dist\ManageInput.exe.
...
$INPUT_SCRIPT = Join-Path $PROJECT_ROOT "manageInput.ahk"
$OUTPUT_DIR = Join-Path $PROJECT_ROOT "dist"
$OUTPUT_EXE = Join-Path $OUTPUT_DIR "ManageInput.exe"
